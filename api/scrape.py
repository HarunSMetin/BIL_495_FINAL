...
class _Scrape:
        ...
        if obj_type == "one-way":
            ...
            if self.data.empty:
                return Scrape(
                    *self.unpack(
                        [
                            [self.origin[i], self.dest[i], date]
                            for i, date in enumerate(self.date)
                        ]
                    ),
                    *self.unpack(
                        [
                            [other.origin[i], other.dest[i], date]
            ...
            if self.origin[0] == other.origin[0]:
                if self.data.empty:
                    return Scrape(
                        *self.unpack(
                            [[self.origin[i], date] for i, date in enumerate(self.date)]
                        ),
                        *self.unpack(
                            [
                                [other.origin[i], date]
    ...
    def __repr__(self):
        ...
        for i, date in enumerate(self._date):
            rep += "{d}: {org} --> {dest}\n".format(
                d=date, org=self._origin[i], dest=self._dest[i]
            )
    ...
    def _set_properties(self, *args):
        ...
        if len(args) == 3:
            assert (
                len(args[0]) == 3 and type(args[0]) == str
            ), "Issue with arg 0, see docs"
            assert (
                len(args[1]) == 3 and type(args[1]) == str
            ), "Issue with arg 1, see docs"
            assert (
                len(args[2]) == 10 and type(args[2]) == str
            ), "Issue with arg 2, see docs"
        ...
        # round-trip
            ...
            assert (
                len(args[3]) == 10 and type(args[3]) == str
            ), "Issue with arg 3, see docs"
            ...
            assert (
                len(self._origin) == len(self._dest) == len(self._date)
            ), "Issue with array lengths, talk to dev"
            self._url = self._make_url()
        ...
        # chain-trip, chain is component of 3s, check that last one is an actual date to not confuse w perfect
            ...
            for i in range(0, len(args), 3):
                assert (
                    len(args[i]) == 3 and type(args[i]) == str
                ), "Issue with arg {}, see docs".format(i)
                assert (
                    len(args[i + 1]) == 3 and type(args[i + 1]) == str
                ), "Issue with arg {}, see docs".format(i + 1)
                assert (
                    len(args[i + 2]) == 10 and type(args[i + 2]) == str
                ), "Issue with arg {}, see docs".format(i + 2)
        ...
        # perfect-chain
            ...
            assert (
                len(args[1]) == 10 and type(args[1]) == str
            ), "Issue with arg 1, see docs"
            ...
            for i in range(2, len(args) - 1, 2):
                ...
                assert (
                    len(args[i + 1]) == 10 and type(args[i + 1]) == str
                ), "Issue with arg {}, see docs".format(i + 1)
                assert datetime.strptime(
                    self._date[-1], date_format
                ) < datetime.strptime(
    ...
    def origin(self, x: str) -> None:
        ...
        ), "Can't set origin after query has been completed."
        self._origin = x
    ...
    def dest(self, x: str) -> None:
        assert (
            self._data.shape[0] == 0
        ), "Can't set destination after query has been completed."
        self._dest = x
    ...
    def date(self, x: str) -> None:
        assert (
            self._data.shape[0] == 0
        ), "Can't set date after query has been completed."
        self.date = x
        ...
        for i in range(len(self._date)):
            urls += [
                "https://www.google.com/travel/flights?hl=en&q=Flights%20to%20{org}%20from%20{dest}%20on%20{date}%20oneway".format(
                    dest=self._dest[i], org=self._origin[i], date=self._date[i]
                )
            ]
        return urls
    ...
    def _clean_results(result, date):
        ...
        matches = [
            i
            for i, x in enumerate(res3)
            if len(x) > 2
            and (
                (x[-2] != "+" and (x.endswith("PM") or x.endswith("AM")) and ":" in x)
                or x[-2] == "+"
            )
        ][::2]
        flights = [
    ...
    def _make_url_request(url, driver):
        ...
        WebDriverWait(driver, timeout=10).until(
            lambda d: len(_Scrape._get_flight_elements(d)) > 100
        )
