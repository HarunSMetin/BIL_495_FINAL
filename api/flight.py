...
class Flight:
    ...
    def _classify_arg(self, arg):
        if ("AM" in arg or "PM" in arg) and len(self._times) < 2 and ":" in arg:
            ...
            self._origin = arg[:3]
            self._dest = arg[3:]
        elif ("hr" in arg and arg[-3:].isupper()) or (
            len(arg.split(", ")) > 1 and arg.isupper()
        ):
            # 1 stop + time at stop
            # or multiple stops
            self._stops = arg
            ...
            len(arg) > 0
            and arg != "Separate tickets booked together"
            and arg != "Change of airport"
        ):
            val = arg.split(",")
